...
const ClassTable = props => {
  ...
  const columns = [
    ...
    {
      title: "Google Class Code",
      dataIndex: "google_class_code",
      key: "google_class_code",
      align: "left",
      showSorterTooltip: false,
      sorter: (a, b) => (+a.google_class_code > +b.google_class_code ? -1 : 1),
      sortOrder: sortedInfo.columnKey === "google_class_code" && sortedInfo.order,
      render(text, record) {
        return {
          children: <div style={{ letterSpacing: 1 }}>{record.google_class_code}</div>
        };
      }
    }
    // {
