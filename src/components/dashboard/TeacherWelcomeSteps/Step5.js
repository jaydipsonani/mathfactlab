...
const Step5 = props => {
  ...
  class ComponentToPrint extends React.Component {
    render() {
      let studentListByActiveClass = studentUserList;
      if (selectedClass?.class_code) {
        studentListByActiveClass = studentUserList.filter(std => selectedClass.class_code === std.class_code);
      }
  ...
  return (
    <>
      ...
      <div className="step-5">
        ...
        <div className="step-5-sub-text">
          {selectedClass.name
            ? `${studentList.length} students have been successfully added to ${selectedClass.name}.`
            : `students have been added successfully.`}
        </div>

        <div className="actions-button">
          <div>
            <Button type="primary" size="large" className="joyride-2" onClick={handlePrint}>
              {selectedClass.name ? `Print Login Card for ${selectedClass.name}` : `Print Login Cards`}
            </Button>
          </div>
        </div>
        <div className="sync-text">
          Login cards can also be found under ‘Actions’ in the top right of the teacher dashboard.
        </div>

        <div className="sync-button">
          <Button type="primary" size="large" className="joyride-2" onClick={() => handleAddMoreClass()}>
      ...
      <div className="welcome-step-popup-footer">
        ...
        {userDetails && userDetails.login_count <= 1 && userDetails.role_id === userRole.TEACHER.role_id ? (
